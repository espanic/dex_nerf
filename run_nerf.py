...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d, dex= False, sigma = 15):
        ...
        def dex_nerf_thresholding(raw, sigma = 15):
            return tf.argmax(raw > sigma, axis= - 1).numpy()
        ...
        # dex_nerf_mask = tf.convert_to_tensor(dex_nerf_mask)
        ...
        # depth estimation proposed by dex-nerf paper. 
        if dex:
            dex_nerf_mask = dex_nerf_thresholding(raw[..., 3], sigma)
            check_all_false = (raw[..., 3] > sigma).numpy().sum(axis = -1) == 0
            dex_nerf_mask[check_all_false] = -1
            
            z_vals_numpy = z_vals.numpy()
            depth_map = [z_vals_numpy[i, j] for i, j in enumerate(dex_nerf_mask)]
            # depth_map = tf.convert_to_tensor(depth_map)
            depth_map = tf.convert_to_tensor(depth_map)
        ...
        # nan values to 0
        disp_map = tf.where(tf.math.is_nan(disp_map), 0, disp_map)
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': depth_map, 'acc_map': acc_map}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if savedir is not None:
            ...
            filename2 = os.path.join(savedir, 'depth_{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
            imageio.imwrite(filename2, disp)
...
def config_parser():
    ...
    # dex
    parser.add_argument('--dex', type=bool, default=False, help='whether we render depth map according to dex-nerf paper')
    parser.add_argument('--sigma', type=float, default=15, help='whether we render depth map according to dex-nerf paper')
    ...
    for i in range(start, N_iters):
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            with writer.as_default():
                ...
                # for step in range(args.i_print):
            ...
            if i % args.i_img == 0:
                ...
                # imageio.imwrite(os.path.join(testimgdir, 'depth_{:06d}.png'.format(i)), to8b( - disp))
